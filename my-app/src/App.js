  ...
  return (
    <div>
      ...
      <TextExpander 
        ...
        buttonColor="#ff6622"
      >
        Space travel is seriously amazing technology and collaboration between countries, private companies, and international space organizations.  And while it's not always easy (or cheap), the results are out of this world.  Think about the first time humans stepped foor on the moon or when rovers were sent to roam around on Mars.
      </TextExpander>

      <TextExpander className="box" expanded={true}>
        Space missions have given us incredible insights into our universe and have inspired future generations to keep reaching for the stars.  Space travel is a pretty cool think to think about.  Who knows what we'll discover next!
      </TextExpander>
    </div>
  );
}
...
function TextExpander({
  ...
  return <div className={className} style={{margin: '10px'}}>
    <span>{displayText}</span>
    <button onClick={() => setIsExpanded(expanded => !expanded)} style={buttonStyle}>{isExpanded ? collapseButtonText : expandButtonText}</button>
  </div>
}
